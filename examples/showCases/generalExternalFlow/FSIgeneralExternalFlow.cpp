...
struct SimulationParameters {
    ...
    std::vector<Array<T,3> > preCICEvertices;
    std::vector<T> areas;
...
void initializeImmersedSurfaceData(SimulationParameters& param)
{
    ...
    param.preCICEvertices.resize(0);
    ...
    for (plint iSurface = 0; iSurface < param.numSurfaces; iSurface++) {
        ...
        ConnectedTriangleSet<T> preciceConnectedTriangleSet(*surfaceTriangleSet);
        ...
        for (plint iVertex = 0; iVertex < numVertices; iVertex++) {
            ...
            param.preCICEvertices.push_back(preciceConnectedTriangleSet.getVertex(iVertex));
            T area;
        ...
        for (plint iVertex = 0; iVertex < 10; iVertex++) {
            plint id = iVertex;
            if (iVertex < 10){
                pcout << "Vertices after scaling: ";
                pcout << "(" << connectedTriangleSet.getVertex(id)[0] << ", " << param.vertices[id][1] << ", " << param.vertices[id][2] << ")" << std::endl;
            }
        }

        surfaceTriangleSet->scale(param.dx);
        surfaceTriangleSet->translate(param.physicalLocation);

        //TriangleSet<T> *CopysurfaceTriangleSet = new TriangleSet<T>(allSurfaceFileNames[iSurface], param.precision);
        ConnectedTriangleSet<T> copyOfConnectedTriangleSet(*surfaceTriangleSet);
        ...
        for (plint iVertex = 0; iVertex < 10; iVertex++) {
            plint id = iVertex;
            if (iVertex < 10){
                pcout << "Vertices after scaling again: ";
                pcout << "(" << copyOfConnectedTriangleSet.getVertex(id)[0] << ", " << copyOfConnectedTriangleSet.getVertex(id)[1] << ", " << copyOfConnectedTriangleSet.getVertex(id)[2] << ")" << std::endl;
            }
        }
        ...
        ConnectedTriangleSet<T> AnothercopyOfConnectedTriangleSet(*surfaceTriangleSet);

        for (plint iVertex = 0; iVertex < 10; iVertex++) {
            plint id = iVertex;
            if (iVertex < 10){
                pcout << "Vertices after scaling a third time: ";
                pcout << "(" << AnothercopyOfConnectedTriangleSet.getVertex(id)[0] << ", " << param.vertices[id][1] << ", " << param.vertices[id][2] << ")" << std::endl;
            }
        }
...
{
    /*
    This is where preCICE can change the position of each vertex on the moving surface.
    The values should be sent here from main()
    */
    for (plint iMovingSurface = 0; iMovingSurface < param.numMovingSurfaces; iMovingSurface++) {
...
int main(int argc, char* argv[])
{
    ...
    // Initialization.
    // Setup preCICE commands
    std::string configFileName = "precice-config.xml";
    std::string solverName = "palabos_Solver";
    std::string meshNameDisp = "palabos_Mesh_disp";
    std::string meshNameForces = "palabos_Mesh_forces";
    
    //int commRank = 0;
    int commSize = global::mpi().getSize();

    int commRank = global::mpi().getRank();

    pcout << "CommSize: " << commSize << " -> and commRank: " << commRank << std::endl;

    SolverInterface interface(solverName, configFileName, commRank, commSize);

    int dispMeshID = interface.getMeshID(meshNameDisp);
    int forcesMeshID = interface.getMeshID(meshNameForces);
    int dimensions = interface.getDimensions();
    int dataDisplacementID = interface.getDataID("Displacements0",dispMeshID);
    int dataForceID = interface.getDataID("Forces0",forcesMeshID);
    int startIdpreCICE;
    ...
    for (plint iMovingSurface = 0; iMovingSurface < param.numMovingSurfaces; iMovingSurface++) {
            N = param.allSurfaces[iMovingSurface].getNumVertices();
            Non0 = 0;
            //if (i == 0){
            //    Non0 = param.allSurfaces[iMovingSurface].getNumVertices();
            //    i++;
           //}
            startIdpreCICE = param.startIds[iMovingSurface];
        }
    ...
    pcout << "Number of moving surfaces: " << param.numMovingSurfaces << std::endl;
    pcout << "Number of vertices: " << N << " and " << Non0 << std::endl;

    std::vector<std::vector<double>> preciceVertices(N, std::vector<double>(dimensions));
    std::vector<std::vector<double>> displacements(N, std::vector<double>(dimensions));
    std::vector<std::vector<double>> allForces(Non0, std::vector<double>(dimensions)); 
    std::vector<int> dataIndicesDisp(Non0,0);
    std::vector<int> dataIndicesForces(Non0,0);

    //for (plint iMovingSurface = 0; iMovingSurface < param.numMovingSurfaces; iMovingSurface++) {
    //    int numVertices = param.allSurfaces[iMovingSurface].getNumVertices();
    //if (global::mpi().getRank() == 0){
        i = 0;

        /*for (plint iVertex = 0; iVertex < N; iVertex++) {
            plint id = iVertex + startIdpreCICE;
            preciceVertices[iVertex][0] = param.preCICEvertices[id][0];
            preciceVertices[iVertex][1] = param.preCICEvertices[id][1];
            preciceVertices[iVertex][2] = param.preCICEvertices[id][2];
            dataIndicesDisp[iVertex] = interface.setMeshVertex(dispMeshID, preciceVertices[iVertex].data());
            if (global::mpi().getRank() == 0){
                dataIndicesForces[iVertex] = interface.setMeshVertex(forcesMeshID, preciceVertices[iVertex].data());
            }
            //if (iVertex < 10){
                        //pcout << "Set Mesh Vertices: ";
                        //pcout << "(" << param.preCICEvertices[id][0] << ", " << param.preCICEvertices[id][0] << ", " << param.preCICEvertices[id][2] << ")" << std::endl;
            //        }
        }
        */

        if (global::mpi().getRank() == 0){
            for (plint iVertex = 0; iVertex < Non0; iVertex++) {
                plint id = iVertex + startIdpreCICE;
                preciceVertices[iVertex][0] = param.preCICEvertices[id][0];
                preciceVertices[iVertex][1] = param.preCICEvertices[id][1];
                preciceVertices[iVertex][2] = param.preCICEvertices[id][2];
        ...
        pcout << "Vertices on IB interface has been set." << std::endl;
    //}
    ...
    for (plint iIter = iniIter; iIter < param.maxIter && !stopExecution; iIter++) {
        ...
        if (iIter != iniIter && (iIter % param.statIter == 0 || iIter == param.maxIter - 1)) {
            pcout << "At iteration " << iIter << ", t = " << iIter * param.dt << std::endl;
                       ...
                       pcout << "dispOne: " << dispOne << std::endl;


        if (interface.isReadDataAvailable()) {
            if (global::mpi().getRank() == 0){ 
                for(int i=0; i<N; i++){
                    interface.readVectorData(dataDisplacementID, dataIndicesDisp[i], displacements[i].data());
                    global::mpi().bCast(&displacements[i][0],1,0);
                    global::mpi().bCast(&displacements[i][1],1,0);
                    global::mpi().bCast(&displacements[i][2],1,0);
                }    
                //displacements[i][0] = 0;
                //displacements[i][1] = 0;
                //displacements[i][2] = 0;
            ...
            pcout << "Displacements have been read from preCICE." << std::endl;
        
            //if (global::mpi().getRank() == 0){
                for (plint iMovingSurface = 0; iMovingSurface < param.numMovingSurfaces; iMovingSurface++) {
                    //plint numVertices = param.allSurfaces[iMovingSurface].getNumVertices();
                    //int startId = param.startIds[iMovingSurface];
                    TriangleSet<T>* triangleSet = param.allSurfaces[iMovingSurface].toTriangleSet(param.precision,
                        &param.vertices, param.startIds[iMovingSurface]);
                    PLB_ASSERT(triangleSet != 0);
                    for (plint iVertex = 0; iVertex < N; iVertex++) {
                        plint id = iVertex + startIdpreCICE;
                        //if (global::mpi().getRank() == 0){
                        param.vertices[id][0] = ((preciceVertices[iVertex][0] + displacements[iVertex][0]) * (1.0 /param.dx)) + (-1.0 * param.physicalLocation[0]/param.dx);
                        param.vertices[id][1] = ((preciceVertices[iVertex][1] + displacements[iVertex][1]) * (1.0 /param.dx)) + (-1.0 * param.physicalLocation[1]/param.dx);
                        param.vertices[id][2] = ((preciceVertices[iVertex][2] + displacements[iVertex][2]) * (1.0 /param.dx)) + (-1.0 * param.physicalLocation[2]/param.dx);
                        //if (iVertex < 10){
                            //pcout << "Vertices after scaling: ";
                            //pcout << "(" << param.vertices[id][0] << ", " << param.vertices[id][1] << ", " << param.vertices[id][2] << ")" << std::endl;
                        //}
                        //}
                    }
                }  
            ...
            instantiateImmersedWallData(param.vertices, param.areas, *container);
            ...
            // Forces on immersed surfaces vertices.
            ...
            //for (plint iSurface = 0; iSurface < 1; iSurface++) {
            for (plint iSurface = 0; iSurface < param.numMovingSurfaces; iSurface++) {
                pcout << "Number of surfaces: " << Non0 << std::endl;
                pcout << "forceConversion: " << forceConversion << std::endl;
                for (plint iVertex = 0; iVertex < N; iVertex++) {
                    //pcout << "N: " << N << " and iVertex = " << iVertex << std::endl;
                    Array<T,3> force = -reduceImmersedForceOnVertex<T>(*container, iSurface, iVertex) * forceConversion;
                    if (global::mpi().getRank() == 0){
                        allForces[iVertex][0] = force[0];
                        allForces[iVertex][1] = force[1];
                        allForces[iVertex][2] = force[2];
                        if (iVertex < 3){
            ...
            for (int i=0; i<Non0; i++){
                interface.writeVectorData(dataForceID, dataIndicesForces[i], allForces[i].data());
            }
            interface.markActionFulfilled(actionWriteIterationCheckpoint());
            pcout << "Forces have been written to preCICE." << std::endl;
           //}


            param.dt = interface.advance(param.dt);
        ...
        if(interface.isTimeWindowComplete() || iIter == iniIter){ // timestep not converged
            pcout << "Time window is complete." << std::endl;
            iterationComplete = 1; // e.g. update variables, increment time
	    }
    }    
        global::timer("lb-iter").start();
        lattice->executeInternalProcessors(); // Execute all processors and communicate appropriately.
