...
/* ******** ReduceImmersedForce3DPerVertex ************************************ */

template<typename T>
ReduceImmersedForce3DOnVertex<T>::ReduceImmersedForce3DOnVertex(int reductionFlag_, int forceID_)
    : sum_g_ids(
            Array<plint,3> (
                this->getStatistics().subscribeSum(),
                this->getStatistics().subscribeSum(),
...
template<typename T>
void ReduceImmersedForce3DOnVertex<T>::processGenericBlocks (
        Box3D domain, std::vector<AtomicBlock3D*> blocks )
{
    PLB_PRECONDITION( blocks.size()==1 );
    AtomicContainerBlock3D* container = dynamic_cast<AtomicContainerBlock3D*>(blocks[0]);
    ...
    //for (pluint i=0; i<vertices.size(); ++i) {
    Array<T,3> vertex = vertices[forceID];
    //sum_g_ids[0]=0.0;
    //sum_g_ids[1]=0.0;
    //sum_g_ids[2]=0.0;
    if ( flags[forceID]==reductionFlag &&
    closedOpenContained(vertex, domain.shift(location.x, location.y, location.z)) )
    {
        this->getStatistics().gatherSum(sum_g_ids[0], g[forceID][0]);
        this->getStatistics().gatherSum(sum_g_ids[1], g[forceID][1]);
        this->getStatistics().gatherSum(sum_g_ids[2], g[forceID][2]);
        //if (forceID < 10){
        //    std::cout << "Forces in function: " << g[forceID][0] << ", " << g[forceID][1] << " -> ";
        //}
    }
...
template<typename T>
ReduceImmersedForce3DOnVertex<T>* ReduceImmersedForce3DOnVertex<T>::clone() const {
    return new ReduceImmersedForce3DOnVertex<T>(*this);
}

template<typename T>
void ReduceImmersedForce3DOnVertex<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
    modified[0] = modif::nothing; // Container Block.
}

template<typename T>
BlockDomain::DomainT ReduceImmersedForce3DOnVertex<T>::appliesTo() const {
    return BlockDomain::bulk;
}

template<typename T>
Array<T,3> ReduceImmersedForce3DOnVertex<T>::getSumG() const {
    return Array<T,3> (
            this->getStatistics().getSum(sum_g_ids[0]),
            this->getStatistics().getSum(sum_g_ids[1]),
            this->getStatistics().getSum(sum_g_ids[2]) );
