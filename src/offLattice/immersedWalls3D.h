...
template<typename T>
class ReduceImmersedForce3DOnVertex : public PlainReductiveBoxProcessingFunctional3D
{
public:
    ReduceImmersedForce3DOnVertex(int reductionFlag_ = 0, int forceID = 0);
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> fields);
    virtual ReduceImmersedForce3DOnVertex<T>* clone() const;
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const;
    virtual BlockDomain::DomainT appliesTo() const;
    Array<T,3> getSumG() const;
private:
...
template<typename T>
Array<T,3> reduceImmersedForceOnVertex(MultiContainerBlock3D& container, int reductionFlag = 0, int forceID = 0)
{
    std::vector<MultiBlock3D*> args;
    args.push_back(&container);
    ReduceImmersedForce3DOnVertex<T> functional(reductionFlag, forceID);
    applyProcessingFunctional(functional, container.getBoundingBox(), args);
    return functional.getSumG();
}
